...
### Answer
You can change the type of the returned value because all the different player types (like HumanPlayer, Randy, and Omola) are subclasses of the Player class that means that they share the same parent type
...
### Answer
The error happens because at compile time Java only knows that the variable player is of Player not the specific subclass like HumanPlayer or Randy.
...
### Answer
This works because of polymorphism and dynamic method dispatch because it calls the same line